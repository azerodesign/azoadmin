import { LayoutDashboard, CheckSquare, Calendar, BarChart2, Users, Settings, HelpCircle, LogOut, ShoppingBag } from 'lucide-react';
import { useBotStatus } from '../../hooks/useBotStatus';

const Sidebar = ({ activePage, setActivePage }) => {
    const { isOnline } = useBotStatus();
    // const isOnline = false; // Fallback for debugging
    ...
    const menuItems = [
        ...
        { id: 'store', icon: ShoppingBag, label: 'Store' }, // Add store back with correct icon
    ];

    return (
        <aside className="w-64 h-screen bg-white flex flex-col fixed left-0 top-0 py-8 px-6 border-r border-gray-100 z-50">
            {/* Logo Area */}
